...
function DataPixelField({
  ...
  const particleTexture = useMemo(() => {
    ...
    gradient.addColorStop(0.25, "rgba(16, 185, 129, 0.9)");
    gradient.addColorStop(0.65, "rgba(6, 182, 212, 0.3)");
  ...
  useFrame(({ clock }) => {
    // Dynamic intensity damping
    if (materialRef.current) {
      materialRef.current.opacity = 0.75 * intensityRef.current;
    }
...
function PredictiveArcs({
  ...
  useFrame(({ clock }) => {
    // Dynamic opacity damping based on route intensity
    const factor = intensityRef.current;
    arcLines.forEach((line) => {
      (line.material as THREE.LineBasicMaterial).opacity = 0.28 * factor;
    });
      ...
      {pulseNodes.map((pos, i) => (
        <mesh key={`node-${i}`} position={pos}>
          ...
          <meshBasicMaterial
            ...
            transparent
            opacity={0.85 * intensityRef.current}
            blending={THREE.AdditiveBlending}
          />
        </mesh>
      ))}
        ...
        {curves.map((_, i) => (
          <mesh key={`pulse-${i}`}>
            ...
            <meshBasicMaterial
              color="#a7f3d0"
              transparent
              opacity={0.9 * intensityRef.current}
              blending={THREE.AdditiveBlending}
            />
          </mesh>
        ))}
...
function ImpactHeart({
  ...
  const heartShape = useMemo(() => {
    ...
    shape.moveTo(0, -0.8);
    shape.bezierCurveTo(-1.4, 0.1, -1.3, 1.2, 0, 1.55);
    shape.bezierCurveTo(1.3, 1.2, 1.4, 0.1, 0, -0.8);
    return shape;
  }, []);
  ...
  useFrame(({ clock }) => {
    ...
    if (heartRef.current) {
      heartRef.current.rotation.z = Math.sin(elapsed * 0.45) * 0.08;
      heartRef.current.scale.setScalar(0.92 + Math.sin(elapsed * 1.4) * 0.035);
      (heartRef.current.material as THREE.MeshBasicMaterial).opacity = 0.12 * intensity;
    }

    if (!reducedMotion && orbitRef.current) {
      orbitRef.current.rotation.z = elapsed * 0.18;
      orbitRef.current.rotation.x = Math.sin(elapsed * 0.25) * 0.12;
    }
  });

  return (
    <group position={[7.8, 2.2, -4.5]} rotation={[0.25, -0.35, 0]}>
      <mesh ref={heartRef} scale={0.7}>
        <extrudeGeometry args={[heartShape, { depth: 0.18, bevelEnabled: true, bevelSegments: 3, bevelSize: 0.06, bevelThickness: 0.05 }]} />
        <meshBasicMaterial color="#34d399" transparent blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
      <group ref={orbitRef}>
        {[0, 1, 2].map((index) => (
          <mesh key={index} position={[Math.cos(index * 2.1) * 1.35, Math.sin(index * 2.1) * 1.35, 0]}>
            <sphereGeometry args={[0.09, 12, 12]} />
            <meshBasicMaterial color={index === 1 ? "#fbbf24" : "#67e8f9"} transparent opacity={0.7} blending={THREE.AdditiveBlending} />
          </mesh>
        ))}
      </group>
    </group>
...
// ==============================================================================
// 3. MAIN SCENE CONTROLLER (Adaptive Intensity, Parallax & Speed Easing)
// ==============================================================================
function SceneController({
  ...
  const speedFactorRef = useRef<number>(1.55); // Starts faster
  const intensityRef = useRef<number>(targetIntensity);
  ...
  useFrame((state, delta) => {
    ...
    // 2. Smooth route intensity damping
    intensityRef.current = THREE.MathUtils.damp(intensityRef.current, targetIntensity, 3.5, delta);

    if (!sceneGroupRef.current) return;

    if (!reducedMotion) {
      // 3. Ambient slow drift
      const driftSpeed = 0.07 * speedFactorRef.current;
      sceneGroupRef.current.rotation.y = Math.sin(elapsed * driftSpeed * 0.5) * 0.07;
      sceneGroupRef.current.rotation.x = Math.cos(elapsed * driftSpeed * 0.4) * 0.03;

      // 4. Damped Mouse Parallax
      const targetRotY = targetPointer.current.x * 0.11;
      const targetRotX = -targetPointer.current.y * 0.08;
      const targetPosX = targetPointer.current.x * 0.7;
      const targetPosY = targetPointer.current.y * 0.4;
  ...
  return (
    <group ref={sceneGroupRef}>
      ...
      <PredictiveArcs
        ...
        reducedMotion={reducedMotion}
      />
      <ImpactHeart intensityRef={intensityRef} reducedMotion={reducedMotion} />
    </group>
  );
}

// ==============================================================================
// 4. GLOBAL 3D BACKGROUND (Persistent Across All Routes)
// ==============================================================================
export default function GlobalBackground() {
  ...
  // Compute target intensity based on active route
  const targetIntensity = useMemo(() => {
    if (pathname === "/") return 1.0; // 100% on Landing
    if (pathname.startsWith("/admin")) return 0.38; // 35–40% on Admin (ensures maximum data readability)
    if (pathname.startsWith("/dashboard")) return 0.52; // 50–55% on Subscriber Dashboard
    return 0.78; // 75–80% on Public pages (/how-it-works, /charities, /pricing, etc.)
  }, [pathname]);
  ...
  return (
    <div
      ...
      style={{
        maskImage: "radial-gradient(ellipse 95% 85% at 50% 40%, black 40%, transparent 100%)",
        WebkitMaskImage: "radial-gradient(ellipse 95% 85% at 50% 40%, black 40%, transparent 100%)",
      }}
    >
      {/* Ambient Cybernetic Gradient Vignette */}
      <div className="absolute inset-0 bg-radial-vignette opacity-70 pointer-events-none" />
      <div
        className="absolute -top-40 -left-40 w-[600px] h-[600px] bg-emerald-500/10 rounded-full blur-[140px] pointer-events-none transition-opacity duration-700"
        style={{ opacity: targetIntensity }}
      />
      <div
        className="absolute top-1/4 -right-40 w-[650px] h-[650px] bg-cyan-500/10 rounded-full blur-[160px] pointer-events-none transition-opacity duration-700"
        style={{ opacity: targetIntensity }}
      />
      <div
        className="absolute -bottom-40 left-1/3 w-[700px] h-[700px] bg-indigo-600/10 rounded-full blur-[180px] pointer-events-none transition-opacity duration-700"
        style={{ opacity: targetIntensity }}
      />
